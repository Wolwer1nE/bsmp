...
__global__ void block_sparse_matvec_kernel(
    ...
    if (matrix_block_idx >= num_nonzero_blocks) return;
...
__global__ void block_sparse_matvec_transpose_kernel(
    ...
    if (local_col >= block_size) return;
    
    int global_col = col_block * block_size + local_col;
    if (global_col >= num_cols) return;
...
void BlockSparseMatrix::freeMemory() {
    if (d_block_rows_) cudaFree(d_block_rows_);
    if (d_block_cols_) cudaFree(d_block_cols_);
    if (d_block_data_) cudaFree(d_block_data_);
...
void BlockSparseMatrix::multiply(const float* d_x, float* d_y, cudaStream_t stream) {
    ...
    if (matrix_blocks_per_cuda_block < 1) matrix_blocks_per_cuda_block = 1;
